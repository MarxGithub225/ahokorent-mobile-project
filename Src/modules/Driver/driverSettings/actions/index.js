...
import { LOGIN, DRIVERSETTINGSSCREEN } from '../../../../common/rootNames';
...
export const Update = (data, props) => async (dispatch) =>{
  ...
  dispatch ({
        type: UPDATEDRIVER_REQUEST
  })
  try {
    const res = await api.update(data);

    dispatch ({
          type: UPDATEDRIVER_SUCCESS
    })
      ...
      _snackSuccess ('Modification réussie');
        ...
        if(result)
        {
          ...
          await AsyncStorage.setItem('islogged', JSON.stringify(user));
            ...
            props.navigation.navigate(DRIVERSETTINGSSCREEN)
        }
